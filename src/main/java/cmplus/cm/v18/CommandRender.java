...
public class CommandRender extends CommandPlusBase {
	...
	private HashMap<String, TickTask> list = new HashMap<String, TickTask>();
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if (args[0].equals("darkscreen")) {
			...
			CMEffect.getEffect().darkScreen(Integer.valueOf(args[2]) * 20, new TickTask() {
				@Override
			...
			if(args[1].equals("minus"))
				CMEffect.getEffect().darkScreen2(Integer.valueOf(args[2]) * 20, 0, new TickTask() {
					@Override
					public void run(Type type) {
						WorldAPI.command(t.findString(args, 3, ""));
					}
		...
		if (args[0].equalsIgnoreCase("drawtext")) {
			...
			if(list.get(name) != null) {
				TickTask absTick = list.get(name);
				absTick.stopTick();
				list.remove(args[1]);
			}
			list.put(name, new TickTask(Type.RENDER, 1, true) {
				@Override
		...
		if (args[0].equalsIgnoreCase("removetext")) {
			TickTask absTick = list.get(args[1].replace("/n/", " "));
			absTick.stopTick();
			list.remove(args[1]);
		}
