...
public class GuiAsdfUI extends GuiScreen {
    static DebAPI back = DebAPI.createDebAPI("back", 15.9, 215.3, 16.5);
    static DebAPI health = DebAPI.createDebAPI("health", 17, 217, 5.8);
    static DebAPI food = DebAPI.createDebAPI("food", 17, 224, 0);
    static DebAPI exp = DebAPI.createDebAPI("exp", 0, 235, 10);
    static DebAPI hotbar = DebAPI.createDebAPI("hot", 48, 26, 8);
    static DebAPI uiback = DebAPI.createDebAPI("ui", 0, 215, 40);
    static DebAPI slot = DebAPI.createDebAPI("slot", 0, 0, 0);
    private DebAPI cur;
    @Override
    public void initGui() {
        super.initGui();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        fontRendererObj.drawString("체력 ",(int) health.x - 10, (int) health.y,0xFFFFFF);
        fontRendererObj.drawString("배고픔 ",(int) food.x - 10, (int) food.y,0xFFFFFF);

        RenderAPI.drawTexture("uibackbar", "asdfrpg:uibackground.png", uiback.x, uiback.y, width, 40).setZ(uiback.z);
        RenderAPI.drawTexture("asdfrpg:backgroundbar.png", back.x, back.y, player.getMaxHealth() * 3, back.z);
        ...
        for (int i = 0; i < 9; i++) {
            ...
            GlStateManager.pushMatrix();
            RenderAPI.drawTexture("slotbar","asdfrpg:backgroundbar.png", slot.x+(i * 20), slot.y,  19,19).setZ(slot.z);
