...
public class EntityAIAttackGolem extends EntityAIBase
{
    World worldObj;
    protected EntityRPGGolem attacker;
    ...
    public EntityAIAttackGolem(EntityRPGGolem creature, double speedIn)
    {
    ...
    public void updateTask()
    {
        ...
        if ((this.attacker.getEntitySenses().canSee(entitylivingbase)) && this.delayCounter <= 0 && (this.targetX == 0.0D && this.targetY == 0.0D && this.targetZ == 0.0D || entitylivingbase.getDistanceSq(this.targetX, this.targetY, this.targetZ) >= 1.0D))
        {
            ...
            if (!this.attacker.getNavigator().tryMoveToXYZ(targetX, targetY, targetZ, this.speed))
            {
