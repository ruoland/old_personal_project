...
public class WeenEffect {
	 ...
	 */
	public static void entityKnockBackDamage(EntityLivingBase base2, double x, double y, double z, float strength, DamageSource source, float damage) {
		for (EntityLivingBase base : base2.worldObj.getEntitiesWithinAABB(EntityLivingBase.class,
				base2.getEntityBoundingBox().expand(x,y,z))) {
			if(base.motionY > 0) {
				System.out.println("넉백 회피함!");
				continue;
			}
			if (base == base2) {
				continue;
