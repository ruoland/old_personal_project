...
public class EntityRopeBlock extends EntityPreBlock {
    ...
    public EntityRopeBlock(World world) {
        super(world);
    ...
    public static double nanum = 10, nanum2 = 0.7, nanum3 = 1.5;//nanum2는 THIRD와 FIRST
    ...
    public void onLivingUpdate() {
        ...
        if (WorldAPI.getPlayerSP() != null) {
            ...
            if(lastChat != null && !prevChat.equalsIgnoreCase(lastChat) && lastChat.split(",").length == 2) {
                prevChat = lastChat;
                String[] split = lastChat.split(",");
                nanum2 = Double.valueOf(split[0]);
                nanum3 = Double.valueOf(split[1]);
            }
            if(lookX == 0 && lookZ == 0) {
                lookReset();
            }
            ...
            if (getCount() == 0 && def.getDistance(first()[0], first()[1], first()[2]) > 0.1) {
                def.setVelocity(firstVelX, firstVelY, firstVelZ);
                if (def.getDistance(first()[0], first()[1], first()[2]) <= 2) {
                    addCount();
                }
            ...
            if (getCount() == 1 && def.getDistance(second()[0], second()[1], second()[2]) > 0.1) {
                def.setVelocity(thirdVelX*nanum2, secondVelY*nanum3, thirdVelZ*nanum2);
                if (def.getDistance(second()[0], second()[1], second()[2]) <= 4) {
                    addCount();
                }
            ...
            if (getCount() == 2 && def.getDistance(third()[0], third()[1], third()[2]) > 0.1) {
                def.setVelocity(thirdVelX, thirdVelY, thirdVelZ);
                if (def.getDistance(third()[0], third()[1], third()[2]) <= 2) {
                    addCount();
                }
            ...
            if (getCount() == 3 && def.getDistance(second()[0], second()[1], second()[2]) > 0.1) {
                def.setVelocity(firstVelX*nanum2, secondVelY*nanum3, firstVelZ*nanum2);
                if (def.getDistance(second()[0], second()[1], second()[2]) <= 4) {
                    resetCount();
                    lookReset();
                }
