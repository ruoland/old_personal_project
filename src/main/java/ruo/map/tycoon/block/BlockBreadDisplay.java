...
public class BlockBreadDisplay extends BlockContainer{
    protected static final AxisAlignedBB AABB_BOTTOM_HALF = new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.5D, 1.0D);

	public BlockBreadDisplay() {
		super(Material.ROCK);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if(hand == EnumHand.MAIN_HAND) {
			TileBreadDisplay tilebread = (TileBreadDisplay) worldIn.getTileEntity(pos);
			BreadData bread = tilebread.getBread();
			if(heldItem != null && Block.getBlockFromItem(heldItem.getItem()) == Blocks.GLASS){
				tilebread.isGlass = true;
