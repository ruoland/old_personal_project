...
public class EntityRoomEnderman extends EntityMR {
    private int moveDelay;
    private static DataParameter<Integer> MAX_DELAY = EntityDataManager.createKey(EntityRoomEnderman.class, DataSerializers.VARINT);
    private static DataParameter<Integer> MOVE_XZ = EntityDataManager.createKey(EntityRoomEnderman.class, DataSerializers.VARINT);
    private static DataParameter<Float> MOVE_X = EntityDataManager.createKey(EntityRoomEnderman.class, DataSerializers.FLOAT);
    private static DataParameter<Float> MOVE_Y = EntityDataManager.createKey(EntityRoomEnderman.class, DataSerializers.FLOAT);
    private static DataParameter<Float> MOVE_Z = EntityDataManager.createKey(EntityRoomEnderman.class, DataSerializers.FLOAT);

    private boolean reverse;

    public EntityRoomEnderman(World worldObj) {
        super(worldObj);
            ...
            if (getMoveXZ() != -1 && moveDelay >= getMaxDelay()) {
                ...
                if (getMoveXZ() == 3 || getMoveXZ() == -1) {
                    ...
                    if(player != null) {
                        ...
                        roomBlock.setPosition(getPositionVector().addVector(0,1,0));
                        worldObj.spawnEntityInWorld(roomBlock);
                        roomBlock.setTarget(player.posX, player.posY, player.posZ);
                        roomBlock.setTargetSpeed(0.5);
                        roomBlock.isFly = true;
                        roomBlock.setBlock(Blocks.EMERALD_BLOCK);
                        roomBlock.setDeathTimer(200);
                    }
                }
