...
import customclient.button.GuiButtonLang;
        ...
        for (GuiButton button : getButtonList()) {
            if (button instanceof GuiCusButton) {
                if (((GuiCusButton) button).canEdit) {
                    if(((GuiCusButton) button).buttonBucket.isScriptEmpty()) {
                        System.out.println("기본 버튼에 스크립트 부여하는 중.");
                        switch (button.displayString) {
                            case "싱글플레이":
                                ((GuiCusButton) button).buttonBucket.read("열기:맵 선택");
                                break;
                            case "멀티플레이":
                                ((GuiCusButton) button).buttonBucket.read("열기:멀티");
                                break;
                            case "Language":
                                ((GuiCusButton) button).buttonBucket.read("열기:언어");
                                break;
                            case "게임 종료":
                                ((GuiCusButton) button).buttonBucket.read("종료");
                                break;
                            case "설정...":
                                ((GuiCusButton) button).buttonBucket.read("열기:설정");
                                break;
                            case "Realms":
                                ((GuiCusButton) button).buttonBucket.read("열기:렐름");
                                break;
                            case "Mods":
                                ((GuiCusButton) button).buttonBucket.read("열기:모드");
                                break;
                            default:
                                ((GuiCusButton) button).buttonBucket.read("내용을 입력하세요.");
                                break;
                        }
                    }
                }
